...
${userDonateStatus}, Не возможно купить отрицательное или нолевое количество крипто валют
                ...
                if (cryptoStatus === true) {
                    ...
                    bot.sendMessage(chatId, `
${userDonateStatus}, Подтвердите оплату с нажав на кнопку ниже
Имя: ${crypto.name}
Кол-во: ${parts[3]}
Стоимость: ${discount}
                    `, { parse_mode: 'HTML', reply_to_message_id: messageId, ...payButton })
                }
                else {
                    bot.sendMessage(chatId, `
...
async function payTransactions(msg, bot, collection) {
    ...
    if (payment === 'paymentCrypto') {
        ...
        if (userCardBalance < globalAmountCrypto) {
            bot.answerCallbackQuery(msg.id, { show_alert: true, text: 'У вас не хватает средств на <b>балансе КАРТЫ !</b>', parse_mode: 'HTML' })
        }
        else {
            bot.editMessageText(`
...
Кол-во: ${globalCountCrypto}
        `, {
            ...
            collection.updateOne({ id: parseInt(userToPayment) }, { $inc: { "bankCard.0.cardValue": -globalAmountCrypto } })
        }
    }
}
